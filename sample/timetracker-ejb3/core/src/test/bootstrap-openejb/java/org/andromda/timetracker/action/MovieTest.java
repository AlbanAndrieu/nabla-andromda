...
public class MovieTest extends JUnitSeamTest // SeamOpenEjbTest
{

	// private static final String JNDI_PREFIX_NAME =
	// SimpleSeamTest.JNDI_PREFIX_NAME;
	...
	private static final String THE_BIG_LEBOWSKI_PLOT = "\"Dude\" Lebowski, mistaken for a millionaire Lebowski, seeks restitution for his "
	...
	private static final String THE_WALL_PLOT = "A troubled rock star descends into madness "
	...
	private static final LogProvider log = Logging
			.getLogProvider(MovieTest.class);

	private boolean mockLogin() {
		Credentials credentials = (Credentials) Component
				.getInstance("org.jboss.seam.security.credentials");
	...
	public void setupClass() throws Exception {
		...
		// Delete all movies
		CrudService crudService = doJndiLookup(/* JNDI_PREFIX_NAME + */"crudService");
		crudService.removeByExample(new Movie(), true);
		assert crudService.findWithType(Movie.class).size() == 0 : "List.size():";

		// Persist 3 movies
		ArrayList<Movie> movies = new ArrayList<Movie>();
		movies.add(new Movie(DIRECTOR_JOEL_COEN, THE_BIG_LEBOWSKI_TITLE,
				THE_BIG_LEBOWSKI_YEAR, "..."));
	...
	public void newMovie() throws Exception {
		...
		new NonFacesRequest("/view/example/Movie.xhtml") {
			...
			protected void renderResponse() {
				...
				assert getValue("#{movieHome.instance.director}").equals(
						THE_WALL_DIRECTOR);
				assert getValue("#{movieHome.instance.title}").equals(
						THE_WALL_TITLE);
				assert getValue("#{movieHome.instance.year}").equals(
						THE_WALL_YEAR);
				assert getValue("#{movieHome.instance.plot}").equals(
						THE_WALL_PLOT);
			}
		}.run();
		...
		new FacesRequest("/view/example/MovieEdit.xhtml") {
			...
			protected void invokeApplication() throws Exception {
				...
				try {
					...
					log.debug("Passed expected unauthorized access failure: "
							+ e.getMessage());
				} finally {
					Conversation.instance().end();
				}
		...
		new FacesRequest("/view/example/MovieEdit.xhtml") {
			...
			protected void invokeApplication() throws Exception {
				...
				try {
					...
					log.debug("Passed expected missing required field value failure: "
							+ e.getMessage());
				} finally {
					Conversation.instance().end();
				}
		...
		new FacesRequest("/view/example/MovieEdit.xhtml") {
			...
			protected void invokeApplication() throws Exception {
				...
				try {
					...
					log.debug("Passed expected duplicate unique secondary index failure: "
							+ e.getMessage());
				} finally {
					Conversation.instance().end();
				}
	...
	public void deleteMovie() throws Exception {
		new FacesRequest("/view/example/MovieEdit.xhtml") {
			...
			protected void invokeApplication() throws Exception {
				...
				invokeMethod("#{movieHome.wire}");
				Assert.assertEquals(invokeMethod("#{movieHome.remove}"),
						"removed", "#{movieHome.remove}");
				Conversation.instance().end();
			}
		}.run();
	}
		...
		new FacesRequest("/view/example/MovieList.xhtml") {
			...
			protected void renderResponse() throws Exception {
				...
				Assert.assertEquals(list.size(), 2,
						"#{movieList.getResultList()}");
				Movie movie = list.get(0);
				Assert.assertEquals(movie.getDirector(), DIRECTOR_JOEL_COEN,
						"movie.getDirector()");
			}
		}.run();
	}
